...
# 2019.11.28
1、config-server自动刷新配置数据
- config-server增加依赖 spring-cloud-starter-bus-amqp spring-boot-starter-security 配置mq消息
- 需要自动刷新配置数据的项目需要增加依赖spring-cloud-starter-bus-amqp、spring-boot-starter-actuator
- 需要自动刷新配置数据的bean需要增加注解@RefreshScope 

2、网关路由配置修改为数据库配置，支持刷新功能 /gatewayRoute/refresh
 - 重写SimpleRouteLocator#locateRoutes方法支持数据库获取路由配置信息
